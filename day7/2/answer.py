...
class FileSystem:
    ...
    def calculateReclaimSpace(self) -> int:
        dir_sizes = []
        for n in range(0, len(self.dag.nodes())):
            size = self.calculateDirSize(n)
            dir_sizes.append((self.dag.nodes[n]['dir'], size))
        
        patch_space = 30000000                    
        free_space = 70000000 - dir_sizes[0][1]   
        reclaim_space = patch_space - free_space  
        min_size = 0                            
        for d in range(0, len(dir_sizes)):
            dir = dir_sizes.pop(0)
            cur_size = dir[1]
            candidate = (cur_size >= reclaim_space)
            if candidate and min_size == 0:
                min_size = cur_size
            elif candidate:
...
print (fs.calculateReclaimSpace() )